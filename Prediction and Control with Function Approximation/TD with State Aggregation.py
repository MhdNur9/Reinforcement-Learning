...
def get_state_feature(num_states_in_group, num_groups, state):
    ...
    for group in range(num_groups):
        if(((group + 1) * num_states_in_group) >= state and (group * num_states_in_group) < state):
            one_hot_vector[group] = 1
...
class TDAgent(BaseAgent):
    ...
    def agent_step(self, reward, state):
        ...
        # ----------------
        # your code here
        last_value = np.dot(self.weights, last_state_feature.T)
        value = np.dot(self.weights, current_state_feature.T)
        self.weights += self.step_size * (reward + self.discount_factor * value - last_value) * last_state_feature
        self.last_state = state
        self.last_action = agent_policy(self.rand_generator, state)
    ...
    def agent_end(self, reward):
        ...
        # ----------------
        # your code here
        self.weights += self.step_size * (reward - np.dot(self.weights, last_state_feature.T)) * last_state_feature
